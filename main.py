...
def download_vods(online_vods: set, base_dir: str, folder="Vods"):
    ...
    for vod_id in todo_vods:
        dowload_vod_cmd = f'twitch-dl download {vod_id}' + ' -q source --output "{id}_{date}.{format}" --max-workers 16'
...
def main():
    for broadcaster in broadcasters:
        working_dir = f"E:\\{broadcaster}\\"
        list_vods_cmd = f'twitch-dl videos {broadcaster} --json --game "Grand Theft Auto V " --all'
