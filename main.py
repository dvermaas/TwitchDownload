...
def download_vods(online_vods: set, folder="Vods"):
    saved_vods = set([filename.split("_")[0] for filename in os.listdir(working_dir+folder)])
    todo_vods = online_vods - saved_vods
    if not todo_vods:
        print("No vods found")
        return
    ts = " ".join(todo_vods)
    dowload_vods_cmd = f'twitch-dl download {ts}' + ' -q source --output "{id}_{date}.{format}" --max-workers 12'
    subprocess.call(dowload_vods_cmd, cwd=working_dir+folder)


def download_chats(online_vods: set, folder="Chats"):
    saved_chats = set([filename.split(".")[0] for filename in os.listdir(working_dir+folder)])
    todo_chats = online_vods - saved_chats
    print(len(todo_chats), "chats found")
